...
import (
	...
	rest_session "github.com/hanzozt/edge-api/rest_client_api_client/session"
	...
	"github.com/hanzozt/sdk-golang/zt/signing"
...
type Context interface {
	...
	// Replaced with event functionality. Use `ztContext.Events().AddMfaTotpCodeListener(func(Context, *rest_model.AuthQueryDetail, MfaCodeResponse))` instead.
...
func (context *ContextImpl) GetTotpCode() <-chan apis.TotpCodeResult {
	...
	if context.ListenerCount(EventMfaTotpCode) == 0 {
		totpCodeResultChan <- apis.TotpCodeResult{
			Code: "",
			Err:  errors.New("no MFA TOTP code providers have been added via ztContext.Events().AddMfaTotpCodeListener()"),
		}
...
func (context *ContextImpl) handleAuthQuery(authQuery *rest_model.AuthQueryDetail) error {
	...
	if *authQuery.Provider == rest_model.MfaProvidersZiti {
		...
		if context.ListenerCount(EventMfaTotpCode) == 0 {
			pfxlog.Logger().Warn("no MFA TOTP code providers have been added via ztContext.Events().AddMfaTotpCodeListener()")
		}
...
func (context *ContextImpl) ListenWithOptions(serviceName string, options *ListenOptions) (edge.Listener, error) {
	...
	return nil, fmt.Errorf("service '%s' not found in zt network", serviceName)
}
...
func (context *ContextImpl) connectEdgeRouter(routerName, ingressUrl string) *edgeRouterConnResult {
	...
	if isGrouped, _ := channel.Headers(underlay.Headers()).GetBoolHeader(channel.IsGroupedHeader); isGrouped {
		...
		multiChannelConfig := &channel.MultiChannelConfig{
			LogicalName:     fmt.Sprintf("zt-sdk[router=%v]", ingressUrl),
			Options:         options,
		...
		ch, err = channel.NewChannelWithUnderlay(fmt.Sprintf("zt-sdk[router=%v]", ingressUrl), underlay, edgeConn, options)
	}
