...
// where multiple Context instances are managed together. Instead of using zt.NewContext() like functions, use
...
//
// ```
// collection := zt.NewCtxCollection()
// cfg, err := zt.NewConfigFromFile(str)
...
// NewContextFromFile is the same as zt.NewContextFromFile but will also add the resulting
...
// NewContextFromFileWithOpts is the same as zt.NewContextFromFileWithOpts but will also add
...
// NewContext is the same as zt.NewContext but will also add the resulting context to the current collection.
func (set *CtxCollection) NewContext(cfg *Config) (Context, error) {
	return set.NewContextWithOpts(cfg, nil)
}

// NewContextWithOpts is the same as zt.NewContextWithOpts but will also add the resulting context to the current
// collection.
