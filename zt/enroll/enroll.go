...
func ParseToken(tokenStr string) (*zt.EnrollmentClaims, *jwt.Token, error) {
...
func ValidateToken(token *jwt.Token) (interface{}, error) {
	...
	claims, ok := token.Claims.(*zt.EnrollmentClaims)
...
func Enroll(enFlags EnrollmentFlags) (*zt.Config, error) {
...
func enrollUpdb(username, password string, token *zt.EnrollmentClaims, caPool *x509.CertPool) (string, error) {
...
func enrollOTT(token *zt.EnrollmentClaims, cfg *zt.Config, caPool *x509.CertPool) error {
...
func enrollCA(token *zt.EnrollmentClaims, cfg *zt.Config, caPool *x509.CertPool) error {
...
func enrollCAAuto(enFlags EnrollmentFlags, cfg *zt.Config, caPool *x509.CertPool) error {
	if id, err := identity.LoadIdentity(cfg.ID); err != nil {
		return err
	} else {
		clientCert := id.Cert()
