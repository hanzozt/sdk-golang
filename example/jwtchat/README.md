...
2) Add an External JWT Signer with a JWKS endpoint
   1) `zt edge create ext-jwt-signer jwtchat-idp "http://localhost:9998" -a hanzozt -u "http://localhost:9998/keys"`
...
3) Create an authentication policy that allows the new `ext-jwt-signer` to authenticate identities
   1) `zt edge create auth-policy jwtchat --primary-ext-jwt-allowed --primary-ext-jwt-allowed-signers <extjwtIdFromStep2>`
   2) Save the resulting `auth-policy` id
4) Create two identities (client, server)
   1) `zt edge create identity service cid1 --external-id cid1 -a jwtchat -P <authPolicyIdFromStep3>`
   2) `zt edge create identity service cid2 --external-id cid2 -a jwtchat -P <authPolicyIdFromStep3>`
5) Create at least one Edge Router
   1) `zt edge create edge-router myRouter <myRouter.yml> -o myRouter.jwt`
   2) `zt router enroll <myRouter.yml> -j myRouter.jwt`
6) Create a service named `jwtchat` with attribute `jwtchat`
   1) `zt edge create service jwtchat -a jwtchat`
7) Creat an Edge Router Policy that gives the new identities access to your Edge Routers
   1) `zt edge create edge-router-policy jwtchat --identity-roles #jwtchat --edge-router-roles #all`
8) Create a Service Edge Router Policy that allows `jwtchat` service usage on your Edge Routers
   1) `zt edge create service-edge-router-policy jwtchat --service-roles #jwtchat --edge-router-roles #all`
9) Create a Service Policy that allows your identities access to the `jwtchat` service
   1) `zt edge create service-policy jwtchatDial Dial --service-roles #jwtchat --identity-roles #jwtchat`
   2) `zt edge create service-policy jwtchatBind Bind --service-roles #jwtchat --identity-roles #jwtchat`
