...
func NewStorage(userStore UserStore) *Storage {
	...
	err := binary.Write(buf, binary.LittleEndian, IntToByteArray(int64(key.E)))

	if err != nil {
		panic(err)
	}

	err = binary.Write(buf, binary.LittleEndian, key.N.Bytes())
...
func (s *Storage) ClientCredentialsTokenRequest(ctx context.Context, clientID string, scopes []string) (op.TokenRequest, error) {
	...
	for _, scope := range scopes {
		...
		if !client.IsScopeAllowed(scope) {
			return nil, fmt.Errorf("scope %s is not allowed for client", scope)
		}
	}

	tokenRequest := &oidc.JWTTokenRequest{
		Issuer:  "",
		Subject: clientID,
		Scopes:  scopes,
		Audience: []string{
