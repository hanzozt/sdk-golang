...
This example demonstrates a ztfied HTTP client.
...
1. Run this script to create everything you need.
    ...
    zt edge create config httpbin.hostv1 host.v1 '{"protocol":"tcp", "address":"httpbin.org","port":80}'
    zt edge create config httpbin.clientv1 intercept.v1 '{"protocols":["tcp"], "addresses":["httpbin.zt"],"portRanges":[{"low":80,"high":80}]}'
 
    echo Create the service
    zt edge create service zt.httpbin --configs "httpbin.hostv1,httpbin.clientv1"
    
    echo Create an identity to make the dial request and enroll it
    zt edge create identity user http-client -a clients -o http-client.jwt
    zt edge enroll --jwt http-client.jwt
    
    echo Create service policies
    zt edge create service-policy zt.httpbin.dial Dial --service-roles "@zt.httpbin" --identity-roles "#clients"
    zt edge create service-policy zt.httpbin.bind Bind --service-roles "@zt.httpbin" --identity-roles "@${ZITI_EDGE_ROUTER}"
    
    echo Create edge router policies
    zt edge create edge-router-policy zt.httpbin-edge-router-policy --edge-router-roles '#all' --identity-roles '#clients,#servers'
    zt edge create service-edge-router-policy zt.httpbin-service-edge-router-policy --edge-router-roles '#all' --service-roles '@zt.httpbin'
    
    echo Run policy advisor to check
    zt edge policy-advisor services
    ```

1. Run the `http-client` example for service `zt.httpbin` using intercept address `tcp:httpbin.zt:80`

    ```bash
    ZITI_IDENTITIES=http-client.json ./http-client http://httpbin.zt
    ```
...
$ ./http-client http://httpbin.zt/json
{
  "slideshow": {
    "author": "Yours Truly", 
